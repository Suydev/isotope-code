...
function getPatchedAuthStoreBundle() {
  try {
    ...
    // browser→supabase.co is unreachable this silently never lands (and the
    // later local-profile update has no avatar in it), so the chat avatar
    // (read from profile_data->>'avatar'/'avatarUrl') never shows the upload.
    // Mirror the data-URL avatar through the LOCAL server /__auth/profile path
    // (server→supabase is always reachable; it uploads to the avatars bucket
    // and stores profile_data.avatar_url/avatar_path). After success we also
    // set the local profile's avatar_url so any later browser push carries it.
    const AV_FROM = 'await m.saveUserProfile(i);const n=va.getState().userId';
    const AV_TO   = 'await m.saveUserProfile(i);try{if(t&&typeof t.avatar==="string"&&t.avatar.indexOf("data:image/")===0){const _arw=localStorage.getItem("isotope-auth-token");const _atk=_arw?(function(){try{return JSON.parse(_arw).access_token}catch(_e0){return _arw}})():_arw||"";fetch("/__auth/profile",{method:"POST",headers:{"Content-Type":"application/json","Authorization":"Bearer "+_atk},body:JSON.stringify({avatar:t.avatar}),signal:AbortSignal.timeout(45000)}).then(function(_r){return _r&&_r.ok?_r.json():null}).then(function(_d){if(_d&&_d.ok&&_d.profile){i.avatar_url=_d.profile.avatar_url||i.avatar_url;if(i.avatar&&i.avatar.indexOf("data:image/")===0){i.avatar=_d.profile.avatar_url||i.avatar}try{m.saveUserProfile(i)}catch(_e1){ }var _x=va.getState().userId;if(_x){Kr(_x,i,s,"full")}}}).catch(function(){})}}catch(_e2){};const n=va.getState().userId';
    ...
    const tryNext = () => {
      ...
      const rq = https.request({
        ...
        r.on('end', () => {
          ...
          // Never write an HTML fallback page (SPA shell) as a JS asset —
          // the upstream app returns HTML for unknown hashed names. Writing it
          // to disk would serve HTML-as-JS and black-screen the app.
          const head8 = body.slice(0, 8).toString('latin1').toLowerCase();
          if (/\.js$/.test(safeName) && (head8.startsWith('<!doctype') || head8.startsWith('<html') || (body.length < 40 && !head8.startsWith('import') && !head8.startsWith('const') && !head8.startsWith('export')))) {
            tryNext();
            return;
          }
...
const PIP_ALLOWED_ACTIONS = new Set(['correct', 'incorrect', 'skipped', 'undo', 'setTarget', 'expand', 'close', 'pause', 'resume', 'togglePause']);
