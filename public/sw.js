...
self.addEventListener('fetch', function(event) {
  ...
  if (reqUrl.origin === self.location.origin && reqUrl.pathname.startsWith('/sounds/')) {
...
self.addEventListener('activate', function(event) {
  var oldRuntimeCaches = {
    'isotope-sounds-v1': true,
    'html-navigation-cache': true,
    'google-fonts-cache': true,
    'gstatic-fonts-cache': true,
    'font-assets-cache': true,
    'images-cache': true,
  ...
  event.waitUntil(
    caches.keys().then(function(keys) {
      return Promise.all(keys.map(function(key) {
        return oldRuntimeCaches[key] ? caches.delete(key) : Promise.resolve(false);
      }));
    })
  );
});
...
// Ambient sounds are fetched from network only by the fetch handler above to
// avoid large local storage growth on phones.
e.registerRoute(
  ({url}) => url.pathname.startsWith('/sounds/'),
  new e.CacheFirst({
    cacheName: 'isotope-sounds-v2',
    plugins: [
      new e.ExpirationPlugin({ maxEntries: 1, maxAgeSeconds: 60 * 60 })
    ]
  })
);
e.cleanupOutdatedCaches(), e.registerRoute(({
        ...
        cacheName: "html-navigation-cache-v2",
        ...
        cacheName: "font-assets-cache-v2",
        plugins: [new e.ExpirationPlugin({
            maxEntries: 6,
            maxAgeSeconds: 604800
        }), new e.CacheableResponsePlugin({
