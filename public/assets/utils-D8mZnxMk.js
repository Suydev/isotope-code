...
const Ge = (e, t) => {
    ...
    Be = e => "isThemeGetter" in e && e.isThemeGetter === !0,
    $e = e => {
        if (e < 1) return {
            get: () => {},
            set: () => {}
    ...
    De = e => {
        ...
        let r = i => {
            ...
            let z = y,
                I = !1;
            y.endsWith(te) ? (z = y.slice(0, -1), I = !0) : y.startsWith(te) && (z = y.slice(1), I = !0);
    ...
    He = (...e) => {
        let t = 0,
            o, r, i = "";
        for (; t < e.length;)(o = e[t++]) && (r = ye(o)) && (i && (i += " "), i += r);
        return i
    },
    ye = e => {
    ...
    ze = /^\((?:(\w[\w-]*):)?(.+)\)$/i,
    Ze = /^\d+\/\d+$/,
    eo = /^(\d+(\.\d+)?)?(xs|sm|md|lg|xl)$/,
    oo = /\d+(%|px|r?em|[sdl]?v([hwib]|min|max)|pt|pc|in|cm|mm|cap|ch|ex|r?lh|cq(w|h|i|b|min|max))|\b(calc|min|max|clamp)\(.+\)|^0$/,
    ro = /^(rgba?|hsla?|hwb|(ok)?(lab|lch)|color-mix)\(.+\)$/,
    to = /^(inset_)?-?((\d+)?\.?(\d+)[a-z]+|0)_-?((\d+)?\.?(\d+)[a-z]+|0)/,
    so = /^(url|image|image-set|cross-fade|element|(repeating-)?(linear|radial|conic)-gradient)\(.+\)$/,
    N = e => Ze.test(e),
